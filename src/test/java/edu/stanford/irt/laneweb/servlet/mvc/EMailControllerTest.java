...
public class EMailControllerTest {
    ...
    public void setUp() throws Exception {
        ...
        this.multipartFiles[0] = this.multipartFile;
    }

    @Test
    public void testFormSubmitEjp() throws IllegalStateException, IOException {
        expect(this.multipartFile.isEmpty()).andReturn(true);
        ...
        this.sender.sendEmail(this.map, new File[] { null });
        replay(this.spamService, this.emailDataBinder, this.sender, this.atts, this.model, this.map,
                this.multipartFile);
        String nextPage = this.controller.formSubmitEJP(this.model, this.multipartFiles, this.atts);
    ...
    @Test
    public void testFormSubmitEJPAttachmentSizeError() throws IllegalStateException, IOException {
        ...
        replay(this.spamService, this.emailDataBinder, this.sender, this.atts, this.model, this.map,
                this.multipartFile);
        assertEquals(ERROR_PAGE, this.controller.formSubmitEJP(this.model, this.multipartFiles, this.atts));
        verify(this.spamService, this.emailDataBinder, this.emailDataBinder, this.sender, this.atts, this.model,
                this.map, this.multipartFile);
    }

    @Test
    public void testFormSubmitEjpContentTypeError() throws IllegalStateException, IOException {
        expect(this.multipartFile.isEmpty()).andReturn(false).times(2);
     ...
     File file = new File("/tmp/filename.jpeg");
     String image =
     "data:image/gif;base64,";
     expect(this.multipartFile.isEmpty()).andReturn(false).times(2);
     expect(this.multipartFile.getContentType()).andReturn("image/jpeg");
     ...
     expect(this.map.put("recipient", "ejproblem@lists.stanford.edu")).andReturn(null);
     expect(this.spamService.isSpam("ejp", this.map)).andReturn(false);
     this.sender.sendEmail(this.map, new File[] { file });
     replay(this.spamService, this.emailDataBinder, this.sender, this.atts, this.model, this.map, this.multipartFile);
     String nextPage = this.controller.formSubmitEJP(this.model, this.multipartFiles, this.atts);
     assertSame(NEXT_EJP_PAGE, nextPage);
     verify(this.spamService, this.emailDataBinder, this.sender, this.atts, this.model, this.map, this.multipartFile);
     }
    ...
    public void testGetParametersMultipleValues() {
        ...
        this.controller.getParameters(this.request, this.model);
        verify(this.spamService, this.emailDataBinder, this.sender, this.atts, this.model, this.request);
    }
