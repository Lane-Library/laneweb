...
public class EngineSearchGeneratorTest {
    ...
    public void setUp() throws Exception {
        ...
        this.generator = new EngineSearchGenerator(this.metaSearchable, this.saxStrategy);
    }

    @SuppressWarnings("unchecked")
    @Test
    public void testDoSearchString() {
        expect(this.metaSearchable.search(isA(Query.class), eq(60000L), (Collection<String>) isNull(), eq(false))).andReturn(
                null);
        replay(this.metaSearchable, this.saxStrategy);
        this.generator.doSearch("query");
        verify(this.metaSearchable, this.saxStrategy);
    }

    @Test
    public void testDoSearchStringEngines() {
        expect(
                this.metaSearchable.search(isA(Query.class), eq(60000L), eq(Arrays.asList(new String[] { "a", "b", "c" })),
