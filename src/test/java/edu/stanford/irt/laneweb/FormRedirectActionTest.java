...
public class FormRedirectActionTest {
    ...
    public void testActBackslash() throws UnsupportedEncodingException {
        ...
        replay(this.params);
        Map result = this.action.act(null, null, null, null, this.params);
        assertEquals("http://lane.stanford.edu/search.html?q=" + URLEncoder.encode("\\cardiology", "UTF-8")
                + "&source=all", result.get("form-redirect-key"));
        verify(this.params);
    }
    ...
    public void testActRegexChars() throws UnsupportedEncodingException {
        ...
        replay(this.params);
        Map result = this.action.act(null, null, null, null, this.params);
        assertEquals("http://lane.stanford.edu/search.html?q=" + URLEncoder.encode("$2cardiology", "UTF-8")
                + "&source=all", result.get("form-redirect-key"));
        verify(this.params);
    }
}
