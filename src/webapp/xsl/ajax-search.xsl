  ...
  <xsl:variable name="keywords" select="/aggregate/s:search/s:query/text()"/>
    ...
    <xsl:template match="h:li">
        ...
        <xsl:copy>
            <xsl:choose>
                <xsl:when test="*">
                    <a type="{$status}" href="{$resource/s:url}"><xsl:value-of select="text()"/></a>&#160;<xsl:copy-of select="*"/><xsl:text>: </xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <a type="{$status}" href="{$resource/s:url}"><xsl:value-of select="text()"/></a><xsl:text>: </xsl:text>
                </xsl:otherwise>
            </xsl:choose>
            <xsl:choose>
