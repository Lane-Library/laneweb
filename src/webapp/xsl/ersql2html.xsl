...
<xsl:stylesheet exclude-result-prefixes="sql" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    version="2.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:sql="http://apache.org/cocoon/SQL/2.0">
    ...
    <xsl:variable name="ltitle"
        select="translate($keywords,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/>
            ...
            <body>
                <xsl:choose>
                    <xsl:when test="sql:row">
                        <dl id="searchResults">
                            <xsl:apply-templates
                                select="sql:row[sql:ltitle = $ltitle
                                    and not(preceding-sibling::sql:row[1]/sql:eresource_id = sql:eresource_id
                                            and preceding-sibling::sql:row[1]/sql:title = sql:title)]"/>
                            <xsl:apply-templates
                                select="sql:row[not(preceding-sibling::sql:row[1]/sql:eresource_id = sql:eresource_id
                                    and preceding-sibling::sql:row[1]/sql:title = sql:title) and sql:ltitle != $ltitle]"/>
                    
                            <xsl:if test="count(sql:row) &gt; 20">
                                <dt>
                                <a href="#">
                                    <img src="/graphics/icons/arrowUpTransp.gif" alt="up" border="0"/>
                                    Back to top </a></dt>
                                <dd/>
                            </xsl:if>
                        </dl>
                    ...
                    <xsl:when test="$a != ''">
                        <div> No results for <xsl:choose>
                            ...
                            </xsl:choose> beginning with <xsl:choose>
                                <xsl:when test="$a = '#'">non alpha character.</xsl:when>
                                <xsl:otherwise><xsl:value-of select="$a"/>.</xsl:otherwise>
                            </xsl:choose> Try the search box above or <a
                                href="http://lmldb.stanford.edu/cgi-bin/Pwebrecon.cgi?DB=local&amp;PAGE=First"
                                >Lane's Catalog</a>
                        </div>
                    </xsl:when>
                </xsl:choose>
    ...
    <xsl:template match="sql:row">
        ...
        <xsl:variable name="links"
            select=".|following-sibling::sql:row[position() &lt; 26 and sql:title/text()=$title and sql:eresource_id/text() = $eresource_id]"/>
        <xsl:variable name="link_count" select="count($links)"/>
        <dt>
            <xsl:value-of select="sql:title/text()"/>
        </dt>
        <dd>
            <ul>
                <xsl:choose>
    ...
    <xsl:template match="sql:description|sql:instruction|sql:publisher">
    ...
    <xsl:template name="multiple-holdings">
        ...
        <xsl:for-each select="$links">
            ...
            <xsl:choose>
                ...
                <xsl:when
                    test="not(following-sibling::sql:row) or following-sibling::sql:row[1]/sql:version_id != $version_id">
                    <xsl:variable name="proxy_class">
                        <xsl:choose>
                            <xsl:when test="sql:proxy = 'T'">proxy</xsl:when>
                            <xsl:otherwise>noproxy</xsl:otherwise>
                ...
                <xsl:otherwise>
                    <xsl:for-each
                        select="self::sql:row[sql:label != 'Get Password'] 
                            | following-sibling::sql:row[position() &lt; 26 and sql:version_id = $version_id and sql:label != 'Get Password']">
                        <xsl:variable name="proxy_class">
                            <xsl:choose>
                                <xsl:when test="sql:proxy = 'T'">proxy</xsl:when>
                                <xsl:otherwise>noproxy</xsl:otherwise>
                        ...
                        <li>
                            <a href="{sql:url}" title="{$title-publisher}" class="{$proxy_class}">
                                <xsl:choose>
                                    <xsl:when
                                        test="preceding-sibling::sql:row[1]/sql:label = 'Get Password'">
                                        <xsl:choose>
                                            <xsl:when
                                                test="$holdings-length &gt; 0 and $dates-length &gt; 0">
                                                <xsl:apply-templates select="sql:holdings"/>,
    ...
    <xsl:template name="multiple-links">
        ...
        <xsl:for-each select="$links">
            <li>
                <a href="{sql:url}" title="{concat(sql:title,':',sql:label)}" class="{$proxy_class}">
                    <xsl:choose>
                        <xsl:when test="string-length(sql:label) &gt; 0">
                            <xsl:value-of select="sql:label"/>
                        </xsl:when>
