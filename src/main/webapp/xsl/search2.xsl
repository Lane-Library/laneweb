...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="attribute::*[contains(.,'{keywords}')]">
    ...
    <!-- can this be removed ... $keywords not used anymore? -->
    ...
    <xsl:template match="h:li[attribute::class='eLibraryTab']">
        <xsl:variable name="genre" select="substring-before(attribute::id,'Tab')"/>
        <xsl:copy>
            <xsl:apply-templates select="attribute::node()[name() != 'class']"/>
            <xsl:if test="/doc/sql:rowset/sql:row[sql:genre=$genre]/sql:hits = '0'">
