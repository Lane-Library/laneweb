...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="h:title">
        <xsl:copy>
            <xsl:choose>
                <xsl:when test="$id">
                    <xsl:value-of
                        select="/h:html/h:body/h:blog/h:entry[@id=$id]/h:ul/h:li[@class='title']"/>
                </xsl:when>
                <xsl:when test="$category">
                    <xsl:value-of select="$category"/>
        ...
        <xsl:choose>
            <xsl:when test="not($id)">
                <xsl:variable name="root-category"
                    select="/h:html/h:body/h:div[@id='categories']/h:ul/h:li[descendant-or-self::h:li/text() = $category]/text()"/>
                <xsl:variable name="root-category-string"
                    select="$category-map/h:div[h:span=$root-category]/h:span[2]"/>
                <div id="leftColumn">
                    <xsl:choose>
                ...
                <div id="mainColumn">
                    ...
                    <dl id="faq">
                        <xsl:apply-templates
                            select="h:entry[h:ul/h:li[@class='categories']/h:ul/h:li/text() = $category]"
                            mode="dl"/>
                    </dl>
                </div>
            </xsl:when>
    ...
    <xsl:template match="h:entry" mode="full">
        ...
        <xsl:variable name="root-category"
            select="/h:html/h:body/h:div[@id='categories']/h:ul/h:li[descendant-or-self::h:li/text() = $primary-category]/text()"/>
        <xsl:variable name="root-category-string"
            select="$category-map/h:div[h:span=$root-category]/h:span[2]"/>
