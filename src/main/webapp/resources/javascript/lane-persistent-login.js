(function() {
	...
	PERSISTENT_EXPIRATION_DAY= 'persistent-expiration-date';
	...
	Y.on('click',function(event) {
		...
		now = new Date(), isActive, 
		persistentStatusCookie = Y.Cookie.get(PERSISTENT_PREFERENCE_COOKIE_NAME);

		if (link && link.get('nodeName') == 'A'	&& (link.get('pathname').indexOf('secure/apps/proxy/credential') > -1 || 
				link.get('host').indexOf('laneproxy') === 0)) {
			if (!persistentStatusCookie		|| ('denied' !== persistentStatusCookie && persistentStatusCookie < now	.getTime())) {
				...
				&& link.get('nodeName') == 'A'	&& (link.get('pathname').indexOf('secure/login.html') > -1)) {
	...
	if (Y.get('#yes-persistent-login')) {
		Y.get('#yes-persistent-login').on(
				'click',
				function(event) {
					if (Y.get('#dont-ask-again') && Y.get('#dont-ask-again').get('checked')) {
						event.preventDefault();
					} else {
						setLink(event); // cookie set in the
						// PerssitentLoginFilter class
					}
				});
	}
	...
	if (Y.get('#no-persistent-login')) {
		Y.get('#no-persistent-login').on('click',function(event) {
					...
					if (Y.get('#dont-ask-again')&& Y.get('#dont-ask-again').get('checked')) {
						...
						Y.Cookie.set(PERSISTENT_PREFERENCE_COOKIE_NAME,
								'denied', {
									path : "/",
									expires : date
								});
	...
	if (Y.get('#dont-ask-again')) {
		Y.get('#dont-ask-again').on('click',
				function(event) {
					if (Y.get('#dont-ask-again') && Y.get('#dont-ask-again').get('checked')) {
						Y.get('#yes-persistent-login').set('src','/../../graphics/buttons/yes-off.gif');
						Y.get('#yes-persistent-login').setStyles({
							cursor : 'default'
						});
					} else {
	...
	var setLink = function(event) {
		...
		url = '/././', userCookie = Y.Cookie.get('user');
