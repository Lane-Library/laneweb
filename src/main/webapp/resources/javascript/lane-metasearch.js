            ...
            getResultCounts: function() {
                ...
                if (LANE.search.getEncodedSearchString() && searchElms.length > 0) {
                    YAHOO.util.Connect.asyncRequest('GET', LANE.search.metasearch.getSearchUrl(), {
                        success: function(o) {
                            var response = YAHOO.lang.JSON.parse(o.responseText), results = response.resources, i, needMore = false, result, updateable, resultSpan, sleepingTime, remainingTime;
                            ...
                            for (i = 0; i < searchables.length; i++) {
                                ...
                                if (result === undefined || !result.status) {
                                    ...
                                    resultSpan.innerHTML = '&#160;' + YAHOO.util.Number.format(result.hits, {
                                        thousandsSeparator: ","
                                    });
                                    if (!updateable.href) {
