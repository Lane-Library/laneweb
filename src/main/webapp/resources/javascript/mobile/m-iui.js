...
//          if (window.iui_ext) window.iui_ext.injectEventMethods(page);    // TG
    ...
    {
        var page = $(pageId), index = 0, backwards = 0;
        if (page)
        {
    ...
    {
        var xhr = new XMLHttpRequest(), data, header;
    ...
    {
        var nodes = frag.childNodes, targetPage, i, child, clone, docNode;
...
{
    var link = findParent(event.target, "a"), form;
    if (link)
    {
...
function sendEvent(type, node, props)
{
    if (node)
    {
        var i, event = document.createEvent("UIEvent");
...
{
    var page,
        result = location.hash.match(/#_([^\?_]+)/);
    if (result)
        page = result[1];
    if (page)
        page = $(page);
...
function checkOrientAndLocation()
{
    ...
    if (!hasOrientationEvent)
    {
      ...
      {   
          ...
          orient = (currentWidth < currentHeight) ? portraitVal : landscapeVal;
    ...
    {
        pageId = location.hash.substr(hashPrefix.length);
...
function updatePage(page, fromPage)
{
    ...
    var pageTitle = $("pageTitle"),
        ttlClass = page.getAttribute("ttlclass"),
        backButton,
        bbClass,
        prevPage,
        homeButton;
    ...
    if (backButton)
    {
        prevPage = $(pageHistory[pageHistory.length-2]);
        if (prevPage && !page.getAttribute("hideBackButton"))
        {
            backButton.style.display = "inline";
            //backButton.innerHTML = prevPage.title ? prevPage.title : "Back";
            backButton.innerHTML = "Back";
            bbClass = prevPage.getAttribute("bbclass");
