...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="h:ul[@class='queryMappingResults']">
        ...
        <xsl:copy>
            ...
            <xsl:for-each select="$items">
                ...
                <!-- web team wants max of 3 resources to display -->
                <xsl:for-each select="$resource-ids[position() &lt;= 3]">
                    <xsl:if test=". = $itemId">
                        <xsl:apply-templates select="$items[@id=$itemId]"></xsl:apply-templates>
                    </xsl:if>
                </xsl:for-each>
            </xsl:for-each>
        </xsl:copy>
