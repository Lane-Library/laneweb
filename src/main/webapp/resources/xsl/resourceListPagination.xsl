...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:function name="fx:pad-string" xmlns:fx="http://lane.stanford.edu/fx">
		...
		<xsl:variable name="paddingLength" select="25"/>
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:sequence select="substring(string-join (($stringToPad, for $i in (1 to $paddingLength) return '.'),''),1,$paddingLength)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
								...
								<select>
                                    ...
                                    <xsl:for-each select="/s:resources/s:pagingLabel">
                                        <xsl:element name="option">
                                        	<xsl:attribute name="value"><xsl:value-of select="concat($request-uri,'?',$no-page-query-string,'page=',position())"/></xsl:attribute>
                                            <xsl:if test="number($page) = position()">
                                                <xsl:attribute name="selected">selected</xsl:attribute>
                                            </xsl:if>
	                                        <xsl:value-of select="concat(fx:pad-string(@start), ' &#8212; ', fx:pad-string(@end), ' (', @results, ')')" />
                                        </xsl:element>
                                    </xsl:for-each>
								</select>
