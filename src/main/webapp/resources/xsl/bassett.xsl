...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:h="http://www.w3.org/1999/xhtml"
	...
	<xsl:template match="h:form[@name='basssett-pagination']/h:input[@name='q']">
		<xsl:if test="$query != ''">
			<xsl:copy>
				<xsl:copy-of select="@*" />
	...
	<xsl:template match="h:form[@name='basssett-pagination']/h:input[@name='r']">
		<xsl:if test="$region != ''">
			<xsl:copy>
				<xsl:copy-of select="@*" />
				<xsl:attribute name="value">
	...
	<xsl:template match="h:form[@name='basssett-pagination']/h:input[@name='pages']">
			<xsl:copy>
				<xsl:copy-of select="@*" />
				<xsl:attribute name="value">
	...
	<xsl:template match="h:form[@name='basssett-pagination']/h:input[@name='t']">
		<xsl:if test="$type != ''">
			<xsl:copy>
				<xsl:copy-of select="@*" />
			</xsl:copy>
