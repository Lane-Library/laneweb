...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:b="http://lane.stanford.edu/bassett/ns" exclude-result-prefixes="h b" version="2.0">
    ...
    <xsl:variable name="thumbnail-directory"><xsl:value-of select="$images-url"
        />/small/</xsl:variable>
    <xsl:variable name="large-image-directory"><xsl:value-of select="$images-url"
        />/large/</xsl:variable>
    <xsl:variable name="medium-image-directory"><xsl:value-of select="$images-url"
        />/medium/</xsl:variable>
    ...
    <xsl:variable name="page-number">
        <xsl:if test="$current-page != ''">
            <xsl:value-of select="number($current-page)"/>
        </xsl:if>
        <xsl:if test="$current-page = ''">
            <xsl:value-of select="number(0)"/>
        </xsl:if>
    </xsl:variable>
    <xsl:variable name="imgs-per-page">
        <xsl:value-of select="number($images-per-page)"/>
    </xsl:variable>

    <xsl:variable name="query-string">
        <xsl:if test="$query != ''">q=<xsl:value-of select="$query"/></xsl:if>
        <xsl:if test="$query != '' and $region != ''">
            <xsl:text>&amp;</xsl:text>
        </xsl:if>
    ...
    <xsl:variable name="modified-query">
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise><xsl:value-of
                        select="/doc/b:bassetts/b:bassett/b:regions/b:region[1]/@b:name"
                    />:</xsl:otherwise>
            </xsl:choose>
        </xsl:copy>
    ...
    <xsl:template match="h:span[@id='bassett-subtitle']">
        <xsl:variable name="sub-titles">
            <xsl:for-each select="/doc/b:bassetts/b:bassett/b:regions/b:region[1]/b:sub_region"
    ...
    <xsl:template match="h:a[@id='photo-choice']/@href">
        <xsl:attribute name="href">
            <xsl:value-of select="."/>
            <xsl:value-of select="$modified-query"/>
        ...
        <xsl:if test="/doc/b:bassetts/b:bassett">
            <xsl:copy>
                ...
                <xsl:for-each select="/doc/b:bassetts/b:bassett">
                    <xsl:if
                        test="position() &gt; $imgs-per-page * $page-number and position() &lt;= $imgs-per-page * $page-number + $imgs-per-page">
                        <div>
                            ...
                            <div>
                                ...
                                <a>
                                    <xsl:attribute name="href">
                                        <xsl:text>/bassett/bassettView.html?bn=</xsl:text>
                                    ...
                                    <img>
                                        ...
                                        <xsl:attribute name="alt">
                                            <xsl:text>bassett Number </xsl:text>
                                            <xsl:value-of select="./@b:bassett_number"/>
                                        </xsl:attribute>
                                    </img>
                                </a>
    ...
    <xsl:template
        match="h:td[@id='diagram-image']/h:a/h:img/@src | h:td[@id='diagram-image']/h:img/@src">
        <xsl:attribute name="src">
            <xsl:value-of select="$image-directory"/>
    ...
    <xsl:template
        match="h:td[@id='legend-image']/h:a/h:img/@src | h:td[@id='legend-image']/h:img/@src">
        <xsl:attribute name="src">
            <xsl:value-of select="$image-directory"/>
    ...
    <xsl:template match="h:*[@class='legend-title']">
        <xsl:copy>
            <xsl:apply-templates select="attribute::node()"/>
            <xsl:value-of
                select="upper-case(substring-before(/doc/b:bassetts/b:bassett/b:title, '.'))"/>
        </xsl:copy>
    ...
    <xsl:template match="h:td[@id='page-number']">
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of
                        select="string($total-images - (($total-pages - $page-number)* $imgs-per-page) )"
                    />
                </xsl:otherwise>
            </xsl:choose>
            <xsl:text> of </xsl:text>
            <xsl:value-of select="$total-images"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="h:a[@id='previous-page']/@href">
        <xsl:attribute name="href">
            ...
            <xsl:if test="$page-number != 0">
                <xsl:text>&amp;pageNumber=</xsl:text>
                <xsl:value-of select="number($page-number - 1)"/>
            </xsl:if>
        </xsl:attribute>
    </xsl:template>
