...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="s:search">
        <html>
            <head>
                ...
                <style type="text/css">
                    h1 {color:olive}
                    div.engine {border:thin olive solid;margin:1em;hight:auto}
                    h2 {background:olive;color:white;margin:0;padding:0}
                    table.engine td{border-bottom:thin olive dashed}
                   
                    table.engine td{padding-right:50px}
                     
                    table.portal td{padding-right:10px;padding-left:10px;border-style:dashed;border-color:olive;border-width:1px } 
            ...
            <body>
                ...
                <xsl:apply-templates select="s:engine">
    ...
    <xsl:template match="s:resource">
            ...
            <tr><td><a href="http://lmldb.stanford.edu/cgi-bin/Pwebrecon.cgi?DB=local&amp;Search_Arg=035a+%28LaneConnex%29+{@s:id}&amp;Search_Code=CMD*&amp;CNT=10"><xsl:value-of select="@s:id"/></a></td> 
