            ...
            <ul>
                <xsl:if test="count(f:facet[@f:key='type']) > 0">
                    ...
                    <xsl:if test="count(f:facet[@f:key='type']) > $values-per-facet">
                        <li>
                            <input name="contains" aria-label="Search by Resource Type" placeholder="Search"
                                data-facet="type" data-searchterm="{$query}" data-facets="{$facets}"
                                class="facet-suggestion" />
                        </li>
                    </xsl:if>
                </xsl:if>
                <xsl:if test="count(f:facet[@f:key='publicationType']) > 0">
                    ...
                    <xsl:if test="count(f:facet[@f:key='publicationType']) > ($values-per-facet -1)">
                        <li>
                            <input name="contains" aria-label="Search by Article Type" placeholder="Search"
                                data-facet="publicationType" data-searchterm="{$query}" data-facets="{$facets}"
                                class="facet-suggestion" />
                        </li>
                    </xsl:if>
                </xsl:if>
                ...
                <li class="facet facetHeader">
                    ...
                    <div>
                        ...
                        <form action="" method="get" id="solr-date-form">
                            <input name="start-year" aria-label="Start Year" type="number" class="date start"
                                min="1400" max="{$current-year}" value="{$start-year}" required="true"
                                placeholder="From" />
                            <input name="end-year" aria-label="End Year" type="number" class="date end" min="1400"
                                max="{$current-year}" value="{$end-year}" required="true" placeholder="To" />
                ...
                <xsl:if test="count(f:facet[@f:key='publicationTitle']) > 0">
                    ...
                    <xsl:if test="count(f:facet[@f:key='publicationTitle']) > $values-per-facet">
                        <li>
                            <input name="contains" aria-label="Search by Journal Title" placeholder="Search"
                                data-facet="publicationTitle" data-searchterm="{$query}" data-facets="{$facets}"
                                class="facet-suggestion" />
                        </li>
                    </xsl:if>
                </xsl:if>
    ...
    <xsl:function name="fct:getLabel">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$name = 'bib'">
                <span class="yui3-tooltip-trigger"
                    title="The journals, books and other resources uniquely available from Lane Medical Library.">Lane Catalog</span>
            </xsl:when>
            <xsl:when test="$name = 'redivis'">
                <span class="yui3-tooltip-trigger"
                    title="Curated datasets provided by Stanford Center for Population Health Sciences hosted on Redivis.">Redivis - PHS</span>
            </xsl:when>
            <xsl:when test="$name = 'web'">
                Lane Web Site
            </xsl:when>
            <xsl:when test="$name = 'class'">relevance
                Lane Classes
            </xsl:when>
            ...
            <xsl:when test="$name = 'dnlm'">
                <span class="yui3-tooltip-trigger"
                    title="A small subset of open access journals from the National Library of Medicine">PMC Journals</span>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:function name="fct:getFacetUrl">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($encoded-facets, concat('::', $facet-id))">
                <xsl:value-of
                    select="concat('&amp;facets=', replace( $encoded-facets, concat('::',$facet-id) , ''))" />
            </xsl:when>
            <xsl:when test="contains($encoded-facets, concat( $facet-id, '::'))">
                <xsl:value-of
                    select="concat('&amp;facets=', replace( $encoded-facets, concat($facet-id,'::') , ''))" />
