        ...
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="number(/doc/r:results/@size) = 0">
                    <xsl:apply-templates
                        select="child::node()[@class='search-summary-none']/child::node()" />
                </xsl:when>
                <xsl:when test="number(/doc/r:results/@size) = 1">
                    <xsl:apply-templates
                        select="child::node()[@class='search-summary-one']/child::node()" />
                </xsl:when>
                <xsl:when test="number(/doc/r:results/@pages) = 1">
                    <xsl:apply-templates
                        select="child::node()[@class='search-summary-more']/child::node()" />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates
                        select="child::node()[@class='search-summary-multipage']/child::node()" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:copy>
    </xsl:template>
