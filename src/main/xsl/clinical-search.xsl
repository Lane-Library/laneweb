  ...
  >
    ...
    <xsl:template match="h:span[starts-with(@r:ref, 'resource@')][ends-with(@r:ref, '/@count')]">
        ...
        <xsl:copy>
            <xsl:apply-templates select="attribute::node()[not(name() = 'r:ref')]"/>
            <xsl:choose>
                <xsl:when test="$content-count"><xsl:value-of select="$content-count"/></xsl:when>
                <xsl:otherwise>?</xsl:otherwise>
            </xsl:choose>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="h:a[@class='clinical-facet'][$facet][substring-after(@href,'facet=') = $facet]">
        <div class="clinical-facet clinical-facet-active">
            <xsl:apply-templates/>
        </div>
    </xsl:template>
    ...
    <xsl:template match="h:span[starts-with(@r:ref, 'resource@')][ends-with(@r:ref, '/@hits')]">
        ...
        <xsl:copy>
            <xsl:apply-templates select="attribute::node()[not(name() = 'r:ref')]"/>
            <xsl:choose>
                <xsl:when test="$hits"><xsl:value-of select="$hits"/></xsl:when>
                <xsl:otherwise>?</xsl:otherwise>
            </xsl:choose>
        </xsl:copy>
    </xsl:template>
