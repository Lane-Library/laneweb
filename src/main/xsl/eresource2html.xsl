...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="s:result[@type='eresource']">
        ...
        <li class="resource">
            ...
            <xsl:if test="($label = 'Book' and contains(s:primaryType, 'Book'))
                or ($label = 'Journal' and contains(s:primaryType, 'Journal'))">
    ...
    <xsl:template match="s:primaryType">
        <xsl:if test="$label and contains('JournalBook',$label)">
            <xsl:choose>
    ...
    <xsl:template name="build-link-label">
        ...
        <xsl:variable name="showLabel">
            <xsl:choose>
                <xsl:when test="not(contains('JournalBook',$label))">true</xsl:when>
