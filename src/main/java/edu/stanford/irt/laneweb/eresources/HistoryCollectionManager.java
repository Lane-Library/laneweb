...
public class HistoryCollectionManager implements CollectionManager {
    ...
    private static final String BROWSE = "SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, TITLE, PUBLISHER, "
            + "HOLDINGS, DATES, DESCRIPTION, PROXY, LABEL, URL, INSTRUCTION, " + "NLSSORT(TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE "
            + "FROM H_ERESOURCE, H_VERSION, H_LINK, H_TYPE " + "WHERE H_ERESOURCE.ERESOURCE_ID = H_VERSION.ERESOURCE_ID "
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_ERESOURCE.ERESOURCE_ID = H_TYPE.ERESOURCE_ID " + "AND H_TYPE.TYPE = ? "
            + "UNION SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, PREFERRED_TITLE AS TITLE, PUBLISHER, "
            + "HOLDINGS, DATES, DESCRIPTION, PROXY, LABEL, URL, INSTRUCTION, " + "NLSSORT(PREFERRED_TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE "
            + "FROM H_ERESOURCE, H_VERSION, H_LINK, H_TYPE " + "WHERE H_ERESOURCE.ERESOURCE_ID = H_VERSION.ERESOURCE_ID "
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_ERESOURCE.ERESOURCE_ID = H_TYPE.ERESOURCE_ID " + "AND H_TYPE.TYPE = ? "
    ...
    private static final String CORE = "SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, TITLE, PUBLISHER, "
            + "HOLDINGS, DATES, DESCRIPTION, PROXY, LABEL, URL, INSTRUCTION, " + "NLSSORT(TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE "
            + "FROM H_ERESOURCE, H_VERSION, H_LINK, H_TYPE " + "WHERE H_ERESOURCE.ERESOURCE_ID = H_VERSION.ERESOURCE_ID "
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_ERESOURCE.CORE = 'Y'AND H_ERESOURCE.ERESOURCE_ID = H_TYPE.ERESOURCE_ID "
            + "AND H_TYPE.TYPE = ? " + "UNION SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, PREFERRED_TITLE AS TITLE, PUBLISHER, "
            + "HOLDINGS, DATES, DESCRIPTION, PROXY, LABEL, URL, INSTRUCTION, " + "NLSSORT(PREFERRED_TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE "
            + "FROM H_ERESOURCE, H_VERSION, H_LINK, H_TYPE " + "WHERE H_ERESOURCE.ERESOURCE_ID = H_VERSION.ERESOURCE_ID "
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_ERESOURCE.CORE = 'Y'AND H_ERESOURCE.ERESOURCE_ID = H_TYPE.ERESOURCE_ID "
            + "AND H_TYPE.TYPE = ? " + "AND PREFERRED_TITLE IS NOT NULL " + "ORDER BY SORT_TITLE, VERSION_ID, LINK_ID";
    ...
    private static final String COUNT = "WITH FOUND AS (SELECT H_ERESOURCE.ERESOURCE_ID, H_TYPE.TYPE, H_SUBSET.SUBSET FROM H_ERESOURCE, H_TYPE, H_SUBSET "
            + "WHERE CONTAINS(H_ERESOURCE.TEXT,?) > 0 " + "AND H_ERESOURCE.ERESOURCE_ID = H_TYPE.ERESOURCE_ID "
            + "AND H_ERESOURCE.ERESOURCE_ID = H_SUBSET.ERESOURCE_ID(+)) " + "SELECT 'all' AS GENRE, COUNT(DISTINCT ERESOURCE_ID) AS HITS FROM FOUND";
    ...
    private static final String MESH = "SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, TITLE, PUBLISHER, "
            ...
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_ERESOURCE.ERESOURCE_ID = H_MESH.ERESOURCE_ID " + "AND H_MESH.TERM = ? "
    ...
    private static final String SEARCH = "WITH FOUND AS ( " + "SELECT TITLE, H_ERESOURCE.ERESOURCE_ID, SCORE(1) * 3 AS SCORE_TEXT, "
            + "CONTAINS(TITLE,?) * 3 AS SCORE_TITLE " + "FROM H_ERESOURCE " + "WHERE CONTAINS(TEXT,?,1) > 0 " + "AND CORE = 'Y' " + "UNION "
            + "SELECT PREFERRED_TITLE AS TITLE, H_ERESOURCE.ERESOURCE_ID, SCORE(1) * 3 AS SCORE_TEXT, " + "CONTAINS(PREFERRED_TITLE,?) * 3 AS SCORE_TITLE "
            + "FROM H_ERESOURCE " + "WHERE CONTAINS(TEXT,?,1) > 0 " + "AND CORE = 'Y' " + "AND PREFERRED_TITLE IS NOT NULL " + "UNION "
            + "SELECT TITLE, H_ERESOURCE.ERESOURCE_ID, SCORE(1) AS SCORE_TEXT, " + "CONTAINS(TITLE,?) AS SCORE_TITLE " + "FROM H_ERESOURCE "
            + "WHERE CONTAINS(TEXT,?,1) > 0 " + "AND CORE IS NULL " + "UNION " + "SELECT PREFERRED_TITLE AS TITLE, H_ERESOURCE.ERESOURCE_ID, "
            ...
            + "SCORE_TITLE, SCORE_TEXT, NLSSORT(TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE " + "FROM FOUND, H_VERSION, H_LINK, H_TYPE, H_SUBSET "
            + "WHERE FOUND.ERESOURCE_ID = H_VERSION.ERESOURCE_ID " + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID "
            + "AND FOUND.ERESOURCE_ID = H_TYPE.ERESOURCE_ID(+) " + "AND H_VERSION.VERSION_ID = H_SUBSET.VERSION_ID(+) "
    ...
    private static final String SEARCH_SUBSET = "WITH FOUND AS ( " + "SELECT TITLE, H_ERESOURCE.ERESOURCE_ID, SCORE(1) * 3 AS SCORE_TEXT, "
            ...
            + "SCORE_TITLE, SCORE_TEXT, NLSSORT(TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE " + "FROM FOUND, H_VERSION, H_LINK, H_TYPE, H_SUBSET "
            + "WHERE FOUND.ERESOURCE_ID = H_VERSION.ERESOURCE_ID " + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID "
            + "AND FOUND.ERESOURCE_ID = H_TYPE.ERESOURCE_ID(+) " + "AND H_VERSION.VERSION_ID = H_SUBSET.VERSION_ID(+) " + "AND SUBSET = ? "
            + "ORDER BY SCORE_TITLE DESC, SCORE_TEXT DESC, SORT_TITLE, VERSION_ID, LINK_ID";
    ...
    private static final String SEARCH_TYPE = "WITH FOUND AS ( " + "SELECT TITLE, H_ERESOURCE.ERESOURCE_ID, SCORE(1) * 3 AS SCORE_TEXT, "
            ...
            + "SCORE_TITLE, SCORE_TEXT, NLSSORT(TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE " + "FROM FOUND, H_VERSION, H_LINK, H_TYPE, H_SUBSET "
            + "WHERE FOUND.ERESOURCE_ID = H_VERSION.ERESOURCE_ID " + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID "
            + "AND FOUND.ERESOURCE_ID = H_TYPE.ERESOURCE_ID(+) " + "AND H_VERSION.VERSION_ID = H_SUBSET.VERSION_ID(+) " + "AND TYPE = ? "
            + "ORDER BY SCORE_TITLE DESC, SCORE_TEXT DESC, SORT_TITLE, VERSION_ID, LINK_ID";

    private static final String SUBSET = "SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, TITLE, PUBLISHER, "
            + "HOLDINGS, DATES, DESCRIPTION, PROXY, LABEL, URL, INSTRUCTION, " + "NLSSORT(TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE "
            + "FROM H_ERESOURCE, H_VERSION, H_LINK, H_SUBSET " + "WHERE H_ERESOURCE.ERESOURCE_ID = H_VERSION.ERESOURCE_ID "
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_VERSION.VERSION_ID = H_SUBSET.VERSION_ID " + "AND H_SUBSET.SUBSET = ? "
            + "UNION SELECT H_ERESOURCE.ERESOURCE_ID, H_VERSION.VERSION_ID, H_LINK_ID, PREFERRED_TITLE AS TITLE, PUBLISHER, "
            + "HOLDINGS, DATES, DESCRIPTION, PROXY, LABEL, URL, INSTRUCTION, " + "NLSSORT(PREFERRED_TITLE,'NLS_SORT=GENERIC_BASELETTER') AS SORT_TITLE "
            + "FROM H_ERESOURCE, H_VERSION, H_LINK, H_SUBSET " + "WHERE H_ERESOURCE.ERESOURCE_ID = H_VERSION.ERESOURCE_ID "
            + "AND H_VERSION.VERSION_ID = H_LINK.VERSION_ID " + "AND H_VERSION.VERSION_ID = H_SUBSET.VERSION_ID " + "AND H_SUBSET.SUBSET = ? "
