...
public class HistoryQueryGenerator extends AbstractGenerator {
    ...
    public void setup(final SourceResolver resolver, final Map objectModel,
        ...
        if (null != this.query) {
            if (this.query.length() == 0) {
                ...
                try {
                    this.translatedQuery = this.queryTranslator
    ...
    public void generate() throws SAXException {
        ...
        if (this.haveParameters) {
            ...
            this.xmlConsumer.startElement(XMLNS, QUERY_ELEMENT, QUERY_ELEMENT,
                    EMPTY_ATTS);
            this.xmlConsumer.characters(selectStatmentChars, 0,
                    selectStatmentChars.length);
    ...
    private void getScoredSelectSQL(final StringBuffer queryBuffer,
            final String titleTable, final boolean coreWeighting) {
        getSelectSQL(queryBuffer, titleTable);
        queryBuffer.append(", CONTAINS(").append(titleTable).append(",'")
                .append(this.translatedQuery).append("',1)");
        if (coreWeighting) {
    ...
    private void getSelectSQL(final StringBuffer queryBuffer,
            final String titleTable) {
        queryBuffer.append(SELECT).append(titleTable).append(", lower(")
                .append(titleTable).append(") AS LTITLE").append(", NLSSORT(")
                .append(titleTable).append(
                        ",'NLS_SORT=GENERIC_BASELETTER') AS STITLE");
    }
    ...
    private void getWhereSQL(final StringBuffer queryBuffer,
            final String titleTable) {
        queryBuffer.append('\n');
        ...
        if (null != this.alpha) {
            ...
            if ("#".equals(this.alpha)) {
                queryBuffer
                        ...
                        .append(titleTable)
                        .append(
                                ",1,1),'NLS_SORT=GENERIC_BASELETTER') > NLSSORT('z','NLS_SORT=GENERIC_BASELETTER'))");
            } else {
                queryBuffer
                        .append("NLSSORT(SUBSTR(")
                        .append(titleTable)
                        .append(
                                ",1,1),'NLS_SORT=GENERIC_BASELETTER') = NLSSORT('")
                        .append(this.alpha).append(
