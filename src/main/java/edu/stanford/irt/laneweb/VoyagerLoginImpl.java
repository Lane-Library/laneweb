...
public class VoyagerLoginImpl extends AbstractLogEnabled implements VoyagerLogin, ThreadSafe, Parameterizable {
    ...
    private String validation(final LDAPPerson ldapPerson, final Request request) {
        ...
        if (request.getParameter("PID") == null || request.getParameter("PID").length() == 0 || !request.getParameter("PID").matches("[0-9a-zA-Z]")) {
            return "PID";
        }
    ...
    public String initPatronSession(final LDAPPerson ldapPerson, final Request request, final Connection conn) throws ProcessingException, SQLException {
    ...
    private boolean clearPatronSession(final Connection conn, String univId, String voyagerPid) throws ProcessingException {
    ...
    private boolean createPatronSession(final Connection conn, String univId, String voyagerPid) throws ProcessingException {
      PreparedStatement stmt = null;
      int rs = -1;
      try {
